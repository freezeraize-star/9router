...
export const PROVIDER_CAPABILITIES = {
  ...
  // Nous — per-model values mirror the gateway's public /v1/models
  // (context_length, top_provider.max_completion_tokens, architecture
  // .input_modalities, reasoning.mandatory). The gateway exposes no web_search
  // parameter, and it accepts OpenAI reasoning_effort on every model while
  // rejecting native vendor thinking fields — so thinkingFormat is "openai"
  // everywhere (also forced at transport level). Exact entries fix the ~latest
  // aliases and ids the generic pattern tables mis-window (e.g. *glm* says
  // 200K; Nous serves 1.31M).
  "nous": {
