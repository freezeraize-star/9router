...
import {
  apiKeyRowToExport, normalizeApiKeyForImport,
  buildApiKeyInsertSql, apiKeyInsertValues,
...
export async function exportDb() {
  ...
  const out = {
    ...
    apiKeys: db.all(`SELECT * FROM apiKeys`).map(apiKeyRowToExport),
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    for (const k of payload.apiKeys || []) {
      // Derived from API_KEY_COLUMNS so the column list, the placeholders, and
      // the values can never drift apart — the previous hand-written pair lost
      // 8 columns on the way back in (limits, usage, rate caps, whitelist).
      db.run(buildApiKeyInsertSql(), apiKeyInsertValues(normalizeApiKeyForImport(k)));
    }
