<div align="center">
  ...
  **Бесплатный AI-провайдер для OpenClaw.**
  ...
  [🚀 Быстрый старт](#-quick-start) • [💡 Возможности](#-key-features) • [📖 Установка](#-setup-guide) • [🌐 Сайт](https://freezeraize.com)
</div>
...
Пакет этого репозитория приватный (`freezeraize-app`), поэтому запуск из исходников/Docker — это ожидаемый путь локальной разработки.
...
| `CLOUD_URL` | `https://freezeraize.com` | Серверный базовый URL эндпоинта облачной синхронизации |
...
| `NEXT_PUBLIC_CLOUD_URL` | `https://freezeraize.com` | Обратно совместимый/публичный облачный URL (приоритет `CLOUD_URL` для серверного рантайма) |
...
- Убедитесь, что `CLOUD_URL` указывает на ожидаемый облачный эндпоинт (например, `https://freezeraize.com`)
...
- Симптом обычно появляется на публичном облачном эндпоинте (`https://freezeraize.com/v1`) для непотоковых (non-streaming) вызовов.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Отправляет OpenAI-совместимый запрос напрямую на облачный эндпоинт (`https://freezeraize.com/v1/chat/completions`) с указанной моделью/ключом.
...
- Публичный облачный эндпоинт (`https://freezeraize.com/v1/chat/completions`):
...
- **Сайт**: [9router.com](https://freezeraize.com)
- **GitHub**: [github.com/freezeraize-star/9router](https://github.com/freezeraize-star/Freezeraize)
...
См. [Pull Requests](https://github.com/freezeraize-star/Freezeraize/pulls) для подробных инструкций.
